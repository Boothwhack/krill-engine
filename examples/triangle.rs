...
use arpg::render::{Color, RenderPass, Target, VertexShader, WGPUContext};
...
fn main() {
    ...
    let context = block_on(WGPUContext::new())
        .expect("WGPU context");
    let mut device = block_on(context.request_device())
        .expect("WGPU device");

    let PhysicalSize { width, height } = window.inner_size();
    let surface = context.create_surface(&window, &device, width, height);
